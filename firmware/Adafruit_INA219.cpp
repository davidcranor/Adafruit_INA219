...
*/
/**************************************************************************/
void Adafruit_INA219::setCalibration_16V_6mA4(void)
{
  // THIS IS CRANOR'S HACKY VERSION THAT MAKES IT WORK WITH A 50ohm SENSE RESISTOR

  // VBUS_MAX = 16V
  // VSHUNT_MAX = 0.32          (Assumes Gain 8, 320mV, can also be 0.16, 0.08, 0.04)
  // RSHUNT = 50               (Resistor value in ohms)
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.00000018
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.00000146

  // 4. Choose an LSB between the min and max values
  //    (Preferrably a roundish number close to MinLSB)
  // CurrentLSB = 0.0000002 (0.2uA per bit)
  ...
  //
  // Max_Current = Current_LSB * 32767
  // Max_Current = 0.00655A before overflow (6.6mA)
  //
  // If Max_Current > Max_Possible_I then
  //    Max_Current_Before_Overflow = MaxPossible_I
  // Else
  ...
  // 8. Compute the Maximum Power
  // MaximumPower = Max_Current_Before_Overflow * VBUS_MAX
  // MaximumPower = 0.0064 * 16V
  // MaximumPower = .1024W

  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 5000;  // Current LSB = 0.2uA per bit (1000/0.2 = 5000)

  //DID NOT UPDATE THIS BIT, AM IN A HURRY AND ONLY CARE ABOUT CURRENT
  ina219_powerDivider_mW = 2;     // Power LSB = 1mW per bit (2/1)
